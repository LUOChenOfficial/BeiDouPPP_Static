...
public:
    ...
    /* MAIN PROCESS */
    /* standard point position(spp)*/
    void pntpos(int epoIndex);
    /* main process */
    void MainProcess();
protected:
    /* TOOL FUNCTION */
    ...
    /* convert ecef to blh*/
    void ecef2blh(const double* ecef, double* blh);
    /* ecef to enu transfromation matrix */
    void xyz2enu(const double* blh, double* E);
    /* compute tropospheric delay by standard atmosphere and saastamoinen model */
    double tropmodel(const double* pos,const double el, double humi);
    /* compute ionospheric delay by broadcast ionosphere model (klobuchar model) */
    double ionmodel(gtime_t t, double* ion, double* pos,double* azel);

    void PrintMatrix(CMatrix A, CString strFileName);
};
