...
void CBdSPP::satazel(double* rr, double* vec,double* azel)
{
	...
	if (rr_blh[2] > -RE_WGS84)
	{
		az = (pow(VecTransMat(0, 0), 2) + pow(VecTransMat(1, 0), 2)) < 1E-12 ? 0.0 : atan2(e/norm, n/norm);
		if (az < 0.0) az += 2 * PI;
		el = asin(u / norm);
	}
	azel[0] = az;
...
void  CBdSPP::ecef2blh(const double* ecef, double* blh)
{
...
void CBdSPP::xyz2enu(const double* blh, double* E)
{
...
double CBdSPP::tropmodel(const double* pos,const double el, double humi)
{
	...
	*/
	...
	for (int index = 0; index < EpochCount; index++)
	{
		...
		for (int i = 0; i < DataEpochCount; i++)
		{
			...
			obssTmp.data[i].P[1] = _tstof(strTmp.Mid(53, 12));
			obssTmp.data[i].L[0] = _tstof(strTmp.Mid(212, 13));
			obssTmp.data[i].L[1] = _tstof(strTmp.Mid(244, 13));
			obssTmp.data[i].rej = 0;
			if (obssTmp.data[i].P[0] == 0 || obssTmp.data[i].P[1] == 0 || obssTmp.data[i].L[0] == 0 || obssTmp.data[i].L[1] == 0)
				obssTmp.data[i].rej = 1;
		...
		for (int m = 0, n = 0; n < BdObsDataCount; m++, n++)
		{
			...
			obss[index].data[n].P[1] = obssTmp.data[m].P[1];
			obss[index].data[n].L[0] = obssTmp.data[m].L[0];
			obss[index].data[n].L[1] = obssTmp.data[m].L[1];
			obss[index].data[n].time = obssTmp.data[m].time;
			obss[index].data[n].sat = obssTmp.data[m].sat;
			obss[index].data[n].prn = _ttoi(obssTmp.data[m].sat.Right(2).Trim());
			obss[index].data[n].EpochIndex = index;
			if (obss[index].data[n].prn <= 18)obss[index].data[n].rej = 1;
			obss[index].data[n].rej = obssTmp.data[m].rej;
		}
	}
...
CMatrix CBdSPP::CalSatPosClkEachEpoch(int epoIndex)
{
		...
		for (int satIndex = 0; satIndex < obss[epoIndex].nmax; satIndex++)
		{
			...
			int prn = _ttoi(obss[epoIndex].data[satIndex].sat.Right(2));
			if (prn == 21)
			{
				double a = 1;
			}
			...
			double a, b, c;
			a = x * cos(L) - y * cos(i) * sin(L);
			b = x * sin(L) + y * cos(i) * cos(L);
			c = y * sin(i);
				...
				obss[epoIndex].data[satIndex].sat == _T("C60"))
			{
				...
				double Z = y * sin(i);
				CMatrix R_x(3, 3), R_z(3, 3);
				R_x(0, 0) = 1;				R_x(0, 1) = 0;				R_x(0, 2) = 0;
				R_x(1, 0) = 0;				R_x(1, 1) = cos(f);	    R_x(1, 2) = sin(f);
				R_x(2, 0) = 0;				R_x(2, 1) = -sin(f);		R_x(2, 2) = cos(f);

				R_z(0, 0) = cos(p);		R_z(0, 1) = sin(p);		R_z(0, 2) = 0;
				R_z(1, 0) = -sin(p);		R_z(1, 1) = cos(p);		R_z(1, 2) = 0;
			...
			double tkTmp = tk;
			for (int i = 0; i < 2; i++)tk = tkTmp - (f0 + f1 * tk + f2 * tk * tk);
		...
		/*
		////////////////////for PPP
		rtk.ssat = new ssat_t[obss[epoIndex].n];
		////////////////////
		*/
		for (int satIndex = 0; satIndex < obss[epoIndex].nmax; satIndex++)
		{
			double rr[4], rs[3], vec[3], az, el, dist, azel[2];
			...
			if ((rs[0] == 0 && rs[1] == 0 && rs[2] == 0) || prn <= 18)
			{
...
void CBdSPP::FormDesConsMatEachEpoch(int epoIndex, CMatrix SatPosClkEpoch,CMatrix& B, CMatrix& L,double*varIon,double*varTrp)
{
		...
		for(int i=0;i<obs.n;i++)
		{
			...
			double el = azel[1];
			///////////////////////for PPP
			memcpy(rtk.ssat[obs.data[ind].prn - 1].azel, azel, sizeof(azel));
			//////////////////////
			dtrp = tropmodel(rr_blh, el, 0.70);
			//varTrp[i] = dtrp * 0.3;
			varTrp[i] = SQR(ERR_SAAS / (sin(el) + 0.1));
			/* Ionosphere delay */
...
void CBdSPP::FormWeightMatEachEpoch(int epoIndex, CMatrix SatPosClkEpoch, CMatrix& P, double* varIon, double* varTrp)
{
	...
	for (int i = 0; i < obs.n; i++)
	{
		...
		satazel(rr, vec, azel);
		el = azel[1];
		if (fabs(el) < minElevAngle(RAD))P(i, i) = 0;
		double p = 0.3, q = 0.3;
		double sigmaURA, sigmaION, sigmaTRP, sigmaELE;
		sigmaELE = p * p + q * q / sin(el);
		//sigmaELE = SQR(100)* (SQR(0.003) + SQR(0.003) / sin(el));
		sigmaION = varIon[i];
		sigmaTRP = varTrp[i];
		// ura value to index
		int uraind = 0;
...
void CBdSPP::pntpos(int epoIndex)
{
	double chisqr[100] = 
	{ 
	...
	while (norm > 1e-4)
	{
		...
		{
			L(i, 0) = L(i, 0) / sqrt(P(i, i));
			for (int j = 0; j < B.Col(); j++)B(i, j) = B(i, j) / sqrt(P(i, i));
		}
		*/
		BT = ~B;
		N = BT *P* B;
		...
		/*
		CString testMat = _T("D://桌面//PPP_Data/testSPPMat.dat");
		PrintMatrix(P, testMat);
		*/
		...
		rrTmp[2] += detX(2, 0);
		rrTmp[3] += detX(3, 0);
		norm = sqrt(pow(detX(0, 0), 2) + pow(detX(1, 0), 2) + pow(detX(2, 0), 2) + pow(detX(3, 0), 2));
	}
	/* evaluate result */
	...
	sol.data[epoIndex].dtr = rrTmp[3] / CLIGHT;
	...
	sol.data[epoIndex].time = timeadd(obss[epoIndex].data[0].time, rrTmp[3] / CLIGHT);
	sol.data[epoIndex].gdop = gdop;
	sol.data[epoIndex].pdop = pdop;
	sol.data[epoIndex].ns = obss[epoIndex].n;
	/* convey to rtk.sol */
	rtk.sol = sol.data[epoIndex];
}

void CBdSPP::MainProcess()
{
	sol.nmax = EpochCount;
	sol.data = new sol_t[EpochCount];
	for (int epoIndex = 0; epoIndex < EpochCount; epoIndex++)
...
void CBdSPP::PrintMatrix(CMatrix A, CString strFileName)
{
	CStdioFile sf;
